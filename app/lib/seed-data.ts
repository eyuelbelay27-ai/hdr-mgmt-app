...
import { buildPermSet } from "./permissions";
 ...
 *
 * Shared between prisma/seed.ts (local `npm run db:seed`) and the one-time
 * production seed trigger (app/api/admin/seed) so both stay in sync.
 */
...
export async function runSeed(prisma: PrismaClient): Promise<{ usersSeeded: number; materialsSeeded: number }> {
  ...
  return { usersSeeded: USERS_SEED.length, materialsSeeded: MATERIALS_SEED.length };
}
